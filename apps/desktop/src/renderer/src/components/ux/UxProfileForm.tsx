...
export function UxProfileForm({
  ...
  return (
    <ScrollableFormShell
      title={profile ? 'Edit appearance profile' : 'New appearance profile'}
      footer={
    ...
    >
      <form id={formId} onSubmit={handleSubmit} className="space-y-3 text-sm">
        ...
        <label className="block">
          ...
          >
            <option value="consoleri">Consolieri (user@host:path)</option>
            <option value="server">Server only</option>
          </select>
          <span className="mt-1 block text-xs text-muted">
            Consolieri supplies a colored prompt when the server has no PS1 configured.
          </span>
        </label>
