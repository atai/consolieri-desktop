...
import type { ConnectionProfile, ConnectivityTestEntry, Host, Report } from '@shared/types'
...
export function ConnectivityReportForm({
  ...
  const [entries, setEntries] = useState<ConnectivityTestEntry[]>(
    report?.config.type === 'connectivity_test' ? report.config.entries : []
  )
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    setError(null)
    try {
      const config = { type: 'connectivity_test' as const, entries }
      if (report) {
  ...
  return (
    <div className="border-b border-[#30363d] bg-[#0d1117] p-4">
      <h3 className="mb-3 text-sm font-medium text-gray-100">
        {report ? 'Edit report' : 'New connectivity test'}
      </h3>
      ...
      <label className="mb-3 block text-sm">
        ...
        <input
          ...
          placeholder="e.g. Production SSH check"
        />
      </label>
