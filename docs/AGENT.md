...
| `npm run version:patch` / `minor` / `major` | bump versions + CHANGELOG + commit `chore(release): vX.Y.Z` (clean tree, git-cliff) |
...
5. **Electron binary + native deps** — `postinstall.mjs` downloads the Electron binary if absent, then runs `electron-rebuild --only node-pty` so PTY matches the Electron ABI. Optional `cpu-features` (via `ssh2`) is skipped — it often fails gyp and is not required. If install scripts were skipped with `--ignore-scripts`, run `npm run install:electron` and `npm run rebuild-native` manually.
